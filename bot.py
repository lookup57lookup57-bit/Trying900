...
BOT_TOKEN = "8997685893:AAFgz9FEUnt0BJd1pBh3HWZ_YWzL7b8II4I"
...
async def add_proxy_db(user_id, proxy_data):
    ...
    for p in user_proxies:
        if p['proxy_url'] == proxy_data['proxy_url']:
            return False
...
async def get_proxy_count(user_id):
    return len(await get_all_user_proxies(user_id))
...
async def check_card_with_retry(card, sites, user_id=None, max_retries=MAX_RETRIES, rotator=None):
    if not sites:
    ...
    last_site = None
    proxy_data = None
    proxies = await get_all_user_proxies(user_id) if user_id else []

    for attempt in range(max_retries):
        if rotator:
            site = rotator.pick_site(sites, exclude=tried)
            if proxies:
                proxy_data = rotator.pick_proxy(proxies)
        else:
            available = [s for s in sites if s not in tried] or list(sites)
            site = random.choice(available)
            if proxies:
                proxy_data = random.choice(proxies)
        tried.add(site)

        result = await check_card_specific_site(card, site, user_id, proxy_data, rotator)
...
async def check_card_specific_site(card, site, user_id=None, proxy_data=None, rotator=None):
    proxy_url = proxy_data.get('proxy_url') if proxy_data else None
    try:
...
** Bot & User Management **
`/add` <site> ⇾ Add site(s) to your DB (also supports .txt files).
`/rm` <site> ⇾ Remove site(s) from your DB.
`/check` ⇾ Test your sites with price filter.
`/info` ⇾ Get your user information.
`/redeem` <key> ⇾ Redeem a premium key.

** Proxy Management (Private Only) **
`/addpxy` <proxy> or reply to .txt – Add proxies (max 200, tests and adds only working).
            ...
            if replied_msg and replied_msg.document:
                ...
                try:
                    async with aiofiles.open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                        content = await f.read()
                    sites_to_add = extract_urls_from_text(content)
                finally:
                    try: os.remove(file_path)
                    except: pass
            elif replied_msg and replied_msg.text:
                sites_to_add = extract_urls_from_text(replied_msg.text)
        add_text = event.raw_text[4:].strip()
        if add_text:
            sites_to_add.extend(extract_urls_from_text(add_text))
        sites_to_add = list(set(sites_to_add))
        if not sites_to_add:
            return await event.reply("❌ 𝙉𝙤 𝙫𝙖𝙡𝙞𝙙 𝙪𝙧𝙡𝙨/𝙙𝙤𝙢𝙖𝙞𝙣𝙨 𝙛𝙤𝙪𝙣𝙙!\n\n💡 Reply to a `.txt` file or type `/add site.com`")
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if replied_msg and replied_msg.document:
                ...
                lines = [l.strip() for l in replied_msg.text.splitlines() if l.strip()]
        else:
            parts = event.raw_text.split(maxsplit=1)
            if len(parts) == 2:
                lines = [l.strip() for l in parts[1].splitlines() if l.strip()]
        if not lines:
            return await event.reply("𝙁𝙤𝙧𝙢𝙖𝙩: /addpxy ip:port:user:pass\n𝙊𝙧 reply to a `.txt` file with proxies.")
        current_count = await get_proxy_count(event.sender_id)
        if current_count >= 200:
            return await event.reply(f"❌ 𝙋𝙧𝙤𝙭𝙮 𝙇𝙞𝙢𝙞𝙩 𝙍𝙚𝙖𝙘𝙝𝙚𝙙! (200/200)")
        max_add = 200 - current_count
        if len(lines) > max_add:
            lines = lines[:max_add]
            await event.reply(f"⚠️ 𝙊𝙣𝙡𝙮 adding first {max_add} proxies (limit reached).")
        parsed = []
        for line in lines:
            pd = parse_proxy_format(line)
            if pd:
                parsed.append(pd)
        if not parsed:
            return await event.reply("❌ 𝙉𝙤 𝙫𝙖𝙡𝙞𝙙 𝙥𝙧𝙤𝙭𝙞𝙚𝙨 𝙛𝙤𝙪𝙣𝙙!")
        testing_msg = await event.reply(f"🔄 𝙏𝙚𝙨𝙩𝙞𝙣𝙜 {len(parsed)} proxies...")
        added = 0
        failed = 0
        for i in range(0, len(parsed), 10):
            batch = parsed[i:i+10]
            tasks = [test_proxy(p['proxy_url']) for p in batch]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for pd, res in zip(batch, results):
                if isinstance(res, tuple) and res[0]:
                    if await add_proxy_db(event.sender_id, pd):
                        added += 1
                else:
                    failed += 1
            await testing_msg.edit(f"🔄 Testing... Added: {added}, Failed: {failed}")
        await testing_msg.edit(f"✅ 𝘼𝙙𝙙𝙚𝙙 {added} proxies, {failed} failed.\n📊 Total: {await get_proxy_count(event.sender_id)}/200")
    except Exception as e:
        await event.reply(f"❌ 𝙀𝙧𝙧𝙤𝙧: {e}")
...
async def remove_proxy(event):
    ...
    try:
        ...
        if arg == 'all':
            ...
            return await event.reply(f"✅ 𝘼𝙡𝙡 {count} proxies removed.")
        try:
            index = int(arg) - 1
            removed = await remove_proxy_by_index(event.sender_id, index)
            if removed:
                await event.reply(f"✅ 𝙍𝙚𝙢𝙤𝙫𝙚𝙙: {removed['ip']}:{removed['port']}\n📊 Remaining: {await get_proxy_count(event.sender_id)}")
            else:
                await event.reply(f"❌ 𝙄𝙣𝙫𝙖𝙡𝙞𝙙 𝙞𝙣𝙙𝙚𝙭! (1-{len(proxies)})")
        except ValueError:
            await event.reply("❌ 𝙄𝙣𝙫𝙖𝙡𝙞𝙙 𝙞𝙣𝙙𝙚𝙭!")
    except Exception as e:
        await event.reply(f"❌ 𝙀𝙧𝙧𝙤𝙧: {e}")
...
async def view_proxy(event):
    ...
    try:
        ...
        if not user_proxies:
            return await event.reply("❌ 𝙔𝙤𝙪 𝙙𝙤𝙣'𝙩 𝙝𝙖𝙫𝙚 𝙖𝙣𝙮 𝙥𝙧𝙤𝙭𝙮 𝙨𝙖𝙫𝙚𝙙!\n\n𝙐𝙨𝙚 /addpxy 𝙩𝙤 𝙖𝙙𝙙.")
        proxy_list = f"📡 **𝙔𝙤𝙪𝙧 𝙋𝙧𝙤𝙭𝙞𝙚𝙨** ({len(user_proxies)}/200)\n\n"
...
async def process_sh_card(event, access_type):
    ...
    try:
        async with sem:
            res, site_index, used_site = await check_card_with_retry(card, user_sites, event.sender_id, MAX_RETRIES, rotator)
        loading_task.cancel()
        end_time = time.time()
...
async def process_msh_cards(event, cards, sites):
    ...
    for i in range(0, len(cards), batch_size):
        ...
        for card in batch:
            tasks.append(
                asyncio.create_task(
                    _run_with_semaphore(sem, check_card_with_retry, card, sites, event.sender_id, MAX_RETRIES, rotator)
                )
            )
        results = await asyncio.gather(*tasks, return_exceptions=True)
...
"""
            ...
            for card in batch:
                ...
                tasks.append(
                    asyncio.create_task(
                        _run_with_semaphore(sem, check_card_with_retry, card, local_sites, user_id, MAX_RETRIES, rotator)
                    )
                )
            ...
            for j, (card, result) in enumerate(zip(task_cards, results)):
                ...
                if is_site_dead(response_text):
                    declined += 1
                    # Do NOT remove site from user DB; just note it's dead
                    if not local_sites:
...
"""
            ...
            for card in batch:
                ...
                tasks.append(
                    asyncio.create_task(
                        _run_with_semaphore(sem, check_card_with_retry, card, global_sites, user_id, MAX_RETRIES, rotator)
                    )
                )
                task_cards.append(card)
            if not tasks:
...
# ── Site Check (/check) – with price filter UI ──
...
async def check_sites(event):
    ...
    if not sites_to_check:
        ...
        sites_to_check = sites_data.get(str(event.sender_id), [])
        if not sites_to_check:
            return await event.reply("❌ 𝙔𝙤𝙪 𝙝𝙖𝙫𝙚𝙣'𝙩 𝙖𝙙𝙙𝙚𝙙 𝙖𝙣𝙮 𝙨𝙞𝙩𝙚𝙨 𝙮𝙚𝙩! Use `/add` first.")

    PENDING_SITE_CHECK_PRICE[event.sender_id] = {
        "sites": sites_to_check,
        "event": event,
        "provided": bool(check_text)
    }

    kb = [
        [Button.inline("💲 0-5 USD", f"checkprice:5:{event.sender_id}"),
         Button.inline("💲 0-10 USD", f"checkprice:10:{event.sender_id}")],
        [Button.inline("💲 0-20 USD", f"checkprice:20:{event.sender_id}"),
         Button.inline("💲 0-40 USD", f"checkprice:40:{event.sender_id}")]
    ]
    await event.reply(
        f"🔍 <b>Site Check with Price Filter</b>\n━━━━━━━━━━━━━━━━━\n"
        f"📌 <b>Sites to check:</b> <code>{len(sites_to_check)}</code>\n"
        f"ℹ️ <i>Select a price range – only working sites within that range will be shown and added to your DB.</i>\n"
        f"━━━━━━━━━━━━━━━━━\n"
        f"👇 <b>Choose range below:</b>",
        buttons=kb,
        parse_mode='html'
    )

@client.on(events.CallbackQuery(pattern=rb"checkprice:(\d+):(\d+)"))
async def check_price_cb(event):
    ...
    data = PENDING_SITE_CHECK_PRICE.pop(uid, None)
    if not data:
        return await event.answer("Expired or invalid!", alert=True)

    await event.answer("Checking sites...", alert=False)
    try:
        await event.delete()
    except:
        pass

    asyncio.create_task(process_site_check_with_price(data["event"], data["sites"], max_price))

async def process_site_check_with_price(event, sites, max_price):
    ...
    final_text = f"""✅ **Site Check Complete!** (≤ ${max_price})

📊 **Results:**
🟢 Working (≤${max_price}): {len(working_sites)}
🔴 Dead / Over-price: {len(dead_sites)}
"""
    if proxy_dead_occurred:
        final_text += "\n⚠️ **Some proxies died during check. They have been removed automatically.**\n"

    if working_sites:
        final_text += "\n✅ **Working Sites (≤${max_price}):**\n"
        for idx, s in enumerate(working_sites, 1):
            final_text += f"{idx}. `{s['site']}` - {s['price']} ({s['gateway']})\n"
        final_text += "\n"
    else:
        final_text += "\n❌ **No working sites found within the selected price range.**\n"

    buttons = []
    if working_sites:
        TEMP_WORKING_SITES[event.sender_id] = [s['site'] for s in working_sites]
    ...
    try:
        ...
        await event.reply(final_text, buttons=buttons, parse_mode='html')
...
✅ 𝙒𝙤𝙧𝙠𝙞𝙣𝙜 (filter): {working_count}
❌ 𝘿𝙚𝙖𝙙/Over: {dead_count}
...
async def add_working_sites_callback(event):
    try:
        ...
        if response_parts:
            response_text = "\n".join(response_parts)
            response_text += f"\n📊 **Total Sites in Your DB:** {len(user_sites)}"
        else:
        ...
        try:
            await event.edit(updated_text, buttons=None, parse_mode='html')
        except:

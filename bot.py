...
SP_PER_USER_WORKERS = 50          # increased
MSP_PER_USER_WORKERS = 200        # increased significantly
SITE_PER_USER_WORKERS = 50        # increased
BIN_WORKERS = 50                  # increased

API_TIMEOUT = 60
BIN_TIMEOUT = 30                  # reduced timeout for bin

BATCH_SIZE = 150                  # increased batch size for fewer iterations
SITE_CHECK_BATCH = 100            # increased
MAX_RETRIES = 3                   # reduced from 4 to speed up

# ────────────────────────────────────────────────────────────────
#  GLOBAL CACHE & SESSION
# ────────────────────────────────────────────────────────────────
_bin_cache = {}                   # cache for BIN info
_http_session = None              # global aiohttp session
...
async def process_site_check(event, sites):
    ...
    for i in range(0, len(sites), batch_size):
        ...
        for site in batch:
            tasks.append(
                asyncio.create_task(
                    _run_with_semaphore(sem, test_single_site, site, user_id=event.sender_id)
                )
            )
        results = await asyncio.gather(*tasks, return_exceptions=True)
